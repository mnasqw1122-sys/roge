...
local PERSONALITIES = {
    aggressive = { name = "狂战士", kite_chance = 0.1, charge_chance = 0.3, combo_chance = 0.2, dodge_chance = 0.05, taunt_chance = 0.1 },
    cautious = { name = "守卫", kite_chance = 0.5, charge_chance = 0.05, combo_chance = 0.05, dodge_chance = 0.2, taunt_chance = 0.15 },
    evasive = { name = "游侠", kite_chance = 0.7, charge_chance = 0.05, combo_chance = 0.1, dodge_chance = 0.35, taunt_chance = 0.02 },
    balanced = { name = "战士", kite_chance = 0.3, charge_chance = 0.15, combo_chance = 0.15, dodge_chance = 0.1, taunt_chance = 0.08 },
}

-- 前向声明：OnLoad 在 StartAITicker 定义之前调用它，需提前声明 local 以便闭包捕获
local StartAITicker
...
local function OnAttacked(inst, data)
    if data.attacker then
        ...
        -- 记录最后一次受到伤害的时间，用于"60秒未受伤害则消失"机制
        inst._last_hit_time = GetTime()
...
local function OnLoad(inst, data)
    if data then
        ...
        inst:DoTaskInTime(0, function()
            ...
            -- 读档后重置受击时间并重启 AI 决策器，使 60 秒超时消失机制继续生效
            inst._last_hit_time = GetTime()
            StartAITicker(inst)
...
function StartAITicker(inst)
    if inst._ai_ticker then return end
    -- 60秒未受到玩家伤害则消失（玩家未击杀即放弃战斗）
    local TIMEOUT_SECONDS = 60
    inst._ai_ticker = inst:DoPeriodicTask(1.0, function()
        if not inst:IsValid() or inst.components.health:IsDead() then return end

        -- 超时消失检查：自最后一次受击起 60 秒未再受伤，则挑战者主动离去
        if inst._last_hit_time and (GetTime() - inst._last_hit_time) >= TIMEOUT_SECONDS then
            if TheWorld and TheWorld.components.rogue_ai_npc_manager then
                TheWorld.components.rogue_ai_npc_manager:OnNPCTimeout(inst)
            else
                inst:Remove()
            end
            return
