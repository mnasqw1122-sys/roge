...
function M.Create(deps)
    ...
    local function ApplyTalentChoice(player, talent_id)
        ...
        data.talent_options = {}
        -- 通过事件总线广播天赋选择，供成就等子系统消费
        EventBus.Emit("on_talent_select", { player = player, talent_id = talent_id })
        -- 广播Build组合检查：天赋变更可能达成 Build 协同事件
        EventBus.Emit("on_build_combo_check", { player = player })
    ...
    S.OfferTalentChoice = OfferTalentChoice
