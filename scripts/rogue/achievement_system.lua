...
function M.Create(deps)
    ...
    -- 通过事件总线订阅关键事件，激活原本定义但从未触发的成就类别：
    -- relic_count / talent_pick / survive_day / shop_spend 在 v2.1.2 前无任何 CheckProgress 调用，
    -- 此处补齐，让"收藏家/遗物大师/天赋觉醒/天赋满载/生存者/豪商"等成就真正可达成。
    EventBus.Subscribe("on_relic_pickup", function(data)
        if data and data.player then
            S.CheckProgress(data.player, "relic_count", 1)
        end
    end)
    EventBus.Subscribe("on_talent_select", function(data)
        if data and data.player then
            S.CheckProgress(data.player, "talent_pick", 1)
        end
    end)
    EventBus.Subscribe("on_shop_purchase", function(data)
        if data and data.player and data.price then
            S.CheckProgress(data.player, "shop_spend", data.price)
        end
    end)
    EventBus.Subscribe("on_season_end", function(data)
        -- 赛季结束时按存活天数推进生存者成就（每位存活玩家）
        if not data or not data.players then return end
        local day_reached = data.day_reached or 0
        for _, p in ipairs(data.players) do
            if p and p:IsValid() and deps.IsValidPlayer(p) then
                S.CheckProgress(p, "survive_day", day_reached)
            end
        end
    end)
        ...
        for _, ach in ipairs(ACHIEVEMENT_DEFS) do
            if ach.kind == kind and not data.achievements[ach.id] then
                ...
                if progress[key] >= ach.target then
                    ...
                    -- 广播成就解锁，供里程碑成就通道等子系统消费
                    EventBus.Emit("on_achievement_unlock", { player = player, achievement_id = ach.id, achievement_name = ach.name })
                end
            end
        end
