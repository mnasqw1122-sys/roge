--[[
    ...
    功能：肉鸽模式的运行时生命周期管理器。
    负责核心内核：持久化钩子、玩家/世界生命周期、击杀/天变事件路由分发，
    并集成事件总线(EventBus)与日志器(Logger)的初始化。
]]
local M = {}
...
function M.Create(deps)
    ...
    -- 注入日志器，使各子系统可通过 deps.Logger 统一输出分级日志
    deps.Logger = Logger

    -- 根据模组配置初始化日志分级（DEBUG_MODE 关闭时仅输出 INFO 及以上）
    if deps.Config and Logger and Logger.SetDebugMode then
        Logger.SetDebugMode(deps.Config.DEBUG_MODE == true)
    end
    -- 同步开启事件总线调试日志，便于排查系统间通信
    if deps.Config and EventBus and EventBus.SetDebugMode then
        EventBus.SetDebugMode(deps.Config.DEBUG_MODE == true)
    end
        ...
        inst.OnSave = function(self, data)
            ...
            if self.rogue_data then
                if deps.Config.DEBUG_MODE then
                    Logger.Debug("Save", "data for " .. tostring(self) .. " kills=" .. tostring(self.rogue_data.kills) .. " dmg=" .. tostring(self.rogue_data.damage_bonus) .. " hp=" .. tostring(self.rogue_data.hp_bonus))
                end
        ...
        inst.OnLoad = function(self, data)
            ...
            if data then
                if data.rogue_growth then
                    if deps.Config.DEBUG_MODE then
                        Logger.Debug("Load", "data for " .. tostring(self) .. " kills=" .. tostring(data.rogue_growth.kills) .. " dmg=" .. tostring(data.rogue_growth.damage_bonus) .. " hp=" .. tostring(data.rogue_growth.hp_bonus))
                    end
                    ...
                    self:DoTaskInTime(0, function()
                        ...
                        if not ok_apply then
                            Logger.Error("Load", "ApplyGrowthState error: " .. tostring(err_apply))
                        end
                        if deps.ReapplyTalentEffects then
                            local ok_talent, err_talent = pcall(deps.ReapplyTalentEffects, self)
                            if not ok_talent then
                                Logger.Error("Load", "ReapplyTalentEffects error: " .. tostring(err_talent))
                            end
                        end
                        if deps.RegisterSetBonusWatcher then
                            ...
                            if not ok_watcher then
                                Logger.Error("Load", "RegisterSetBonusWatcher error: " .. tostring(err_watcher))
                            end
                        end
                        if deps.RefreshSetBonuses then
                            local ok_set, err_set = pcall(deps.RefreshSetBonuses, self)
                            if not ok_set then
                                Logger.Error("Load", "RefreshSetBonuses error: " .. tostring(err_set))
                            end
                        end
                    end)
                end
    ...
    local function HookWorldPersistence(inst)
        ...
        inst.OnSave = function(self, data)
            ...
            if deps.ExportMilestoneState then
                data.rogue_world.milestone = deps.ExportMilestoneState()
            end
        end
        ...
        inst.OnLoad = function(self, data)
            ...
            if deps.ImportMilestoneState then
                deps.ImportMilestoneState(rw.milestone)
            end
    ...
    S.RegisterPlayerLifecycle = function()
        deps.AddPlayerPostInit(function(inst)
            ...
            -- 函数说明：全局词缀"治疗压制"消费端——玩家受到的治疗效果降低。
            -- 仅包装该玩家实例的 health.DoDelta（正值即治疗），不影响其他生物。
            if inst.components.health and not inst.rogue_heal_suppress_hooked then
                inst.rogue_heal_suppress_hooked = true
                local health = inst.components.health
                local old_DoDelta = health.DoDelta
                health.DoDelta = function(self, amount, cause, causefn, afflicter)
                    if amount and amount > 0 and deps.GetGlobalModifier then
                        local suppress = deps.GetGlobalModifier("global_heal_suppress") or 0
                        if suppress > 0 then
                            amount = amount * (1 - suppress)
                        end
                    end
                    return old_DoDelta(self, amount, cause, causefn, afflicter)
                end
            end
            ...
            inst:DoTaskInTime(0, function()
                ...
                -- 函数说明：资源收集类日常任务（kind=8）——玩家获得非装备物品时计数。
                -- 通过 inst.rogue_init 跳过开局初始发放，避免污染任务进度。
                if not inst.rogue_collect_listener then
                    inst.rogue_collect_listener = true
                    inst:ListenForEvent("itemget", function(player, data)
                        if not deps.ProgressDailyTaskOnCollect then return end
                        if not player.rogue_init then return end
                        local item = data and data.item
                        if item and item:IsValid() and not item.components.equippable then
                            deps.ProgressDailyTaskOnCollect(player, 1, deps.GetCurrentDay())
                        end
                    end)
                end

                -- 函数说明：无伤战斗类日常任务（kind=10）——受击时标记当日已受伤。
                if not inst.rogue_attacked_listener then
                    inst.rogue_attacked_listener = true
                    inst:ListenForEvent("attacked", function(player)
                        if deps.MarkDailyTaskDamageTaken then
                            deps.MarkDailyTaskDamageTaken(player)
                        end
                    end)
                end
                ...
                if not inst.rogue_kill_listener then
                    ...
                    inst:ListenForEvent("killed", function(doer, data)
                        ...
                        if deps.ProgressDailyTaskOnCombo then
                            local d2 = deps.EnsurePlayerData(doer)
                            deps.ProgressDailyTaskOnCombo(doer, d2.combo_count or 0, day)
                        end
