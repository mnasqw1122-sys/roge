...
function M.Create(deps)
    ...
    state.DropLoot = function(victim, is_boss, day, killer)
        ...
        if is_boss then
            ...
            if gear_pool then
                ...
                if has_greed then extra_chance = extra_chance + 0.3 end
                -- 全局词缀"Boss连战"：额外掉落加成
                local global_extra = deps.GetGlobalModifier and deps.GetGlobalModifier("global_boss_extra_drop") or 0
                local gauntlet_killer = killer and deps.EnsurePlayerData(killer) or nil
                if global_extra > 0 then
                    extra_chance = math.min(0.95, extra_chance + global_extra)
                elseif gauntlet_killer and (gauntlet_killer.global_boss_extra_drop or 0) > 0 then
                    extra_chance = math.min(0.95, extra_chance + gauntlet_killer.global_boss_extra_drop)
                end
                if math.random() < extra_chance then
                    local extra = BossDropSystem.PickBossGearBySignature(gear_pool, victim, objective_boost) or deps.PickWeightedCandidate(gear_pool, gear_w)
                    if extra then
        ...
        -- 全局词缀"掉落饥荒"：物品掉落率降低
        local loot_penalty = deps.GetGlobalModifier and deps.GetGlobalModifier("global_loot_penalty") or 0
        if loot_penalty > 0 then chance = chance - loot_penalty end
    ...
    -- opts.no_min_day = true 时忽略天数解锁限制（重铸为付费服务，可重铸出高级能力）
    state.GetAvailableAbilitiesForItem = function(item, day, opts)
        if not item or not item:IsValid() then return {} end
        ...
        local available = {}
        for _, def in ipairs(ABILITY_DEFS) do
            if (opts and opts.no_min_day or day >= (def.min_day or 0))
               and RaritySystem.IsAbilitySlotMatch(item, def.slot)
               and not existing[def.id] then
                table.insert(available, {
                    id = def.id,
