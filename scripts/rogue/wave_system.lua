...
function M.Create(deps)
    ...
    local function SpawnEnemy(player, is_boss, day, opts)
        ...
        if is_boss then
            ...
            if not candidate then
                ...
                -- 全局词缀"精英潮涌"：精英出现概率提升
                local global_elite_bonus = deps.GetGlobalModifier and deps.GetGlobalModifier("global_elite_surge") or 0
                local elite_chance = math.min(deps.CONST.MAX_ELITE_CHANCE, deps.Config.ELITE_CHANCE + math.floor((day - 1) / 25) * 0.02 + rule_bonus + route_bonus + global_elite_bonus)
            ...
            if ent then
                ...
                if ent.components.health then
                    if is_boss then
                        ...
                        local hp = 0
                        -- 全局词缀"Boss强化"：Boss生命提升
                        local boss_hp_mult = deps.GetGlobalModifier and deps.GetGlobalModifier("global_boss_hp_mult") or 1
                        if opts.trial_boss then
                        ...
                        ent.components.health:SetMaxHealth(hp * boss_hp_mult)
                ...
                if ent.components.combat then
                    ...
                    -- [V2.3] 全局词缀/环境词缀统一缩放：作用于本波次生成的所有敌人
                    local gmod = deps.GetGlobalModifier
                    if gmod then
                        local blood_moon = gmod("env_blood_moon_active")
                        local night_bonus = gmod("global_night_damage_bonus") or 0
                        local world = GetWorld()
                        local is_night = world ~= nil and world.state ~= nil and world.state.isnight == true
                        if ent.components.combat and ent.components.combat.defaultdamage then
                            local dmg = ent.components.combat.defaultdamage
                            local gdmg_mult = gmod("global_enemy_damage_mult") or 1
                            local gboss_dmg_mult = is_boss and (gmod("global_boss_dmg_mult") or 1) or 1
                            if blood_moon then dmg = dmg * 1.15 end
                            if gdmg_mult ~= 1 then dmg = dmg * gdmg_mult end
                            if gboss_dmg_mult ~= 1 then dmg = dmg * gboss_dmg_mult end
                            if night_bonus > 0 and is_night then dmg = dmg * (1 + night_bonus) end
                            if dmg ~= ent.components.combat.defaultdamage then
                                ent.components.combat:SetDefaultDamage(dmg)
                            end
                        end
                        if ent.components.health then
                            local hp_mult = 1
                            if blood_moon then hp_mult = hp_mult * 1.1 end
                            hp_mult = hp_mult * (1 - (gmod("global_enemy_hp_penalty") or 0))
                            if hp_mult ~= 1 then
                                ent.components.health:SetMaxHealth(ent.components.health.maxhealth * hp_mult)
                                ent.components.health:SetPercent(1)
                            end
                        ...
                        if ent.components.locomotor then
                            local speed_mult = gmod("global_enemy_speed_mult") or 1
                            if speed_mult ~= 1 then
                                ent.components.locomotor:SetExternalSpeedMultiplier(ent, "rogue_global_speed", speed_mult)
                            end
                        end
                        -- 全局词缀"铁皮"：敌人受击减伤（动态添加 armor 组件，仅对无护甲敌人）
                        local dr = gmod("global_enemy_damage_reduction") or 0
                        if dr > 0 and not ent.components.armor then
                            ent:AddComponent("armor")
                            ent.components.armor:InitIndestructible(dr)
                        end
                    end
    ...
    local function StartWave(day, opts)
        ...
        if is_boss_wave then
            ...
            -- 全局词缀"Boss连战"：每层额外生成1名Boss（掉落加成在Boss击杀结算处消费）
            local gauntlet = deps.GetGlobalModifier and deps.GetGlobalModifier("global_boss_gauntlet") or 0
            count = count + gauntlet
            for i = 1, count do
