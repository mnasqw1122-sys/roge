...
function M.Create(deps)
    ...
    -- 函数说明：成就通道判定——任一存活玩家达成全部指定成就即视为解锁（全队共享奖励）。
    local function CheckAchievementMilestoneUnlocked(ach_reqs)
        if not ach_reqs or #ach_reqs == 0 then return false end
        for _, p in ipairs(deps.CollectAlivePlayers()) do
            if p.rogue_data and p.rogue_data.achievements then
                local all_ok = true
                for _, aid in ipairs(ach_reqs) do
                    if not p.rogue_data.achievements[aid] then
                        all_ok = false
                        break
                    end
                end
    ...
    S.CheckMilestones = function(day)
        ...
        for _, milestone_def in ipairs(milestone_defs) do
            if not IsMilestoneClaimed(milestone_def.id) then
                ...
                if milestone_def.require_achievements then
                    -- 成就解锁通道（方向A - Task A4）：天数达标 + 任一玩家达成指定成就
                    unlocked = day >= milestone_def.day and CheckAchievementMilestoneUnlocked(milestone_def.require_achievements)
                else
                    -- 赛季进程通道（天数 + 击杀/Boss/精英指标）
                    unlocked = day >= milestone_def.day and CheckMilestoneUnlocked(milestone_def.id, metrics)
                end
                if unlocked then
    ...
    S.GetMilestoneStates = function()
        ...
        for _, milestone_def in ipairs(milestone_defs) do
            ...
            table.insert(states, {
                ...
                day_reached = false,
                achievements_required = milestone_def.require_achievements,
    ...
    -- 订阅成就解锁事件：成就通道里程碑即时判定（任一玩家达成成就后立刻结算）
    EventBus.Subscribe("on_achievement_unlock", function()
        if deps.GetCurrentDay then
            S.CheckMilestones(deps.GetCurrentDay())
        end
    end)
