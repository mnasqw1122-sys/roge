...
M.SEASON_MILESTONE_DEFS = {
    ...
    -- 成就解锁通道（方向A - Task A4）：任一存活玩家达成指定成就即可解锁（全队发奖）
    { id = 5, name = "群星璀璨", day = 20, desc = "成就通道：有玩家达成「灭绝者」", require_achievements = { "exterminator_500" }, rewards = { mat_count = 4, gear_chance = 0.40, relic_chance = 0.15 } },
    { id = 6, name = "集大成者", day = 40, desc = "成就通道：有玩家达成Boss统治者+连杀传说+遗物大师", require_achievements = { "boss_dominion", "combo_30", "relic_6" }, rewards = { mat_count = 6, gear_chance = 0.50, relic_chance = 0.25 } },
}
...
M.SEASON_MILESTONE_UNLOCK_METRICS = {
    ...
    { id = 5, min_kills = 0, min_bosses = 0, min_elites = 0 },
    { id = 6, min_kills = 0, min_bosses = 0, min_elites = 0 },
}
