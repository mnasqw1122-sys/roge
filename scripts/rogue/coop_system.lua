...
function M.Create(deps)
    ...
    local function RemoveCoopBuff(player, buff_id)
        ...
        elseif buff_id == "defense" then
            local def = BUFF_LOOKUP[buff_id]
            if player.components.health then
                player.components.health.externalabsorbmultiplier = (player.components.health.externalabsorbmultiplier or 1) - (player.rogue_coop_stacks and player.rogue_coop_stacks[buff_id] or 0) * (def and def.bonus or 0.05)
            end
        elseif buff_id == "speed" then
            if player.components.locomotor then
    ...
    S.RefreshCoopBuffs = function()
        ...
        -- 清理不在协作范围内的玩家（鬼魂/掉线）的残留增益
        for _, p in ipairs(deps.GLOBAL.AllPlayers or {}) do
            if p and p:IsValid() and (p:HasTag("playerghost") or not p:HasTag("player")) and p.rogue_coop_stacks then
                S.CleanupPlayerCoopBuffs(p)
            end
        end
