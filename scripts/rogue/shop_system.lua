...
function M.Create(deps)
    ...
    -- 函数说明：安全获取武器伤害数值，兼容动态伤害武器（如三叉戟，weapon.damage 为函数）。
    local function GetWeaponDamageValue(inst, fallback)
        local dmg = inst.components and inst.components.weapon and inst.components.weapon.damage
        if type(dmg) == "function" then
            local ok, v = pcall(dmg, inst)
            if ok and type(v) == "number" then return v end
            return fallback or 0
        end
        return (type(dmg) == "number" and dmg) or fallback or 0
    end

    -- 函数说明：将伤害加成应用到武器上。
    -- 动态伤害武器（weapon.damage 为函数）：包装原函数，在每次计算伤害时叠加加成；
    -- 普通武器（weapon.damage 为数值）：记录原始伤害后 SetDamage 叠加。
    local function ApplyDamageBonusToWeapon(item, total_bonus)
        local w = item.components.weapon
        if not w then return end
        local dmg = w.damage
        if type(dmg) == "function" then
            if not item._rogue_dmg_func_wrapped then
                item._rogue_dmg_func_wrapped = true
                local orig_fn = dmg
                w.damage = function(inst, attacker, target)
                    local base = orig_fn(inst, attacker, target)
                    local bonus = inst.rogue_upgrade_stats and inst.rogue_upgrade_stats.damage_bonus or 0
                    return (type(base) == "number" and base or 0) + bonus
                end
            end
        else
            local base_dmg = item._rogue_original_damage or (type(dmg) == "number" and dmg or 0)
            item._rogue_original_damage = base_dmg
            w:SetDamage(base_dmg + total_bonus)
        end
    end
    ...
    local function RestoreUpgradesFromSave(item)
        ...
        for stat_id, stat_val in pairs(item.rogue_upgrade_stats) do
            ...
            elseif stat_id == "damage_bonus" and item.components.weapon then
                ApplyDamageBonusToWeapon(item, stat_val)
                ...
                if item.components.weapon and not item._rogue_upgrade_crit_hooked then
                    ...
                    item.components.weapon:SetOnAttack(function(inst, attacker, target)
                        ...
                        if chance > 0 and math.random() < chance and target and target.components.health and not target.components.health:IsDead() then
                            local base_dmg = GetWeaponDamageValue(inst, 34)
    ...
    local function ApplyUpgradeToItem(item, upgrade_def, value)
        ...
        if upgrade_def.id == "damage_bonus" and item.components.weapon then
            ...
            ApplyDamageBonusToWeapon(item, total_bonus)
    ...
    S.BuyItem = function(player, item_id)
        ...
        -- 通过事件总线广播商店购买，供成就等子系统消费
        EventBus.Emit("on_shop_purchase", { player = player, item = item_def.prefab, price = final_cost })
    ...
    S.BlackMarketReforge = function(player)
        ...
        if deps.DropSystem and deps.DropSystem.GetAvailableAbilitiesForItem then
            -- 重铸为付费服务：不受天数解锁限制，可重铸出任意槽位匹配能力
            available = deps.DropSystem.GetAvailableAbilitiesForItem(target_item, day, { no_min_day = true })
        end
        ...
        -- 按权重随机选择新能力（weight 越大越常见），保证重铸结果有变化
        local total_weight = 0
        for _, ab in ipairs(candidates) do
            total_weight = total_weight + (ab.weight or 1)
        end
        local roll = math.random() * total_weight
        local acc = 0
        local new_ability = candidates[#candidates]
        for _, ab in ipairs(candidates) do
            acc = acc + (ab.weight or 1)
            if roll <= acc then
                new_ability = ab
                break
            end
