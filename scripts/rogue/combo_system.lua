...
function M.Create(deps)
    ...
    local function RefreshComboState(player, data)
        ...
        -- 全局词缀"连击风暴"：连击窗口 +50%（world级词缀优先，兼容历史玩家数据）
        local global_window = data.global_combo_window_mult or (deps.GetGlobalModifier and deps.GetGlobalModifier("global_combo_window_mult")) or 1
        local window = (CONST.COMBO_BASE_WINDOW + (data.combo_window_bonus or 0)) * global_window
    ...
    function S.ReleaseComboSkill(player, skill_id)
        ...
        -- [V2.3] 广播连击技能释放事件（日常任务 kind=13 等消费）
        local EventBus = require("rogue/event_bus")
        EventBus.Emit("on_combo_skill", { player = player, skill_id = skill_id })
    end
