...
function M.Create(deps)
    ...
    local function ApplyEliteAffix(ent, day, force)
        ...
        -- 全局词缀"双词缀"：精英双词缀概率提升
        local double_bonus = deps.GetGlobalModifier and deps.GetGlobalModifier("global_double_affix_bonus") or 0
        local second_chance = math.min(deps.CONST.MAX_SECOND_AFFIX_CHANCE, deps.Config.SECOND_AFFIX_CHANCE + math.floor((day - 1) / 35) * 0.03 + double_bonus)
    ...
    -- [V3 扩展] 给 Boss 附加专属词缀
    -- 随机抽取一个满足 min_day 条件的 Boss 词缀并应用其属性与行为
    local function ApplyBossAffix(ent, day, force)
        if not ent or not ent:IsValid() or not ent.components or not ent.components.health then return false end
        if ent:HasTag("rogue_boss_affix") then return false end

        local defs = deps.BOSS_AFFIX_DEFS or {}
        local candidates = {}
        local total_weight = 0
        for _, affix in ipairs(defs) do
            if day >= (affix.min_day or 1) then
                table.insert(candidates, affix)
                total_weight = total_weight + (affix.weight or 1)
            end
        end
        if #candidates == 0 then return false end
        if not force and math.random() > (deps.Config.BOSS_AFFIX_CHANCE or 0.4) then return false end

        local roll = math.random() * total_weight
        local acc = 0
        local chosen = candidates[#candidates]
        for _, affix in ipairs(candidates) do
            acc = acc + (affix.weight or 1)
            if roll <= acc then
                chosen = affix
                break
            end
        ...
        -- 应用属性加成
        ApplyAffixStats(ent, chosen)

        -- 应用 Boss 专属行为
        if chosen.id == "boss_berserk" then
            -- 怒涛：血量低于50%时进入狂暴，伤害+40%持续10秒
            if not ent.rogue_boss_berserk_hooked then
                ent.rogue_boss_berserk_hooked = true
                ent:ListenForEvent("healthdelta", function(inst)
                    ...
                    if inst.components.health:GetPercent() <= 0.5 and not inst.rogue_boss_berserk_active then
                        inst.rogue_boss_berserk_active = true
                        if inst.components.combat and inst.components.combat.externaldamagemultipliers then
                            inst.components.combat.externaldamagemultipliers:SetModifier(inst, 1.4, "rogue_boss_berserk")
                        end
                        inst:DoTaskInTime(10, function(i)
                            if i and i:IsValid() and i.components.combat and i.components.combat.externaldamagemultipliers then
                                i.components.combat.externaldamagemultipliers:RemoveModifier(i, "rogue_boss_berserk")
                                i.rogue_boss_berserk_active = false
                            end
                        end)
                    end
                end)
            end
        elseif chosen.id == "boss_doppel" then
            -- 残影：每30秒分裂一个自身镜像（镜像属性50%）
            if not ent.rogue_boss_doppel_task then
                ent.rogue_boss_doppel_task = ent:DoPeriodicTask(30, function(inst)
                    if not inst:IsValid() or not inst.components.health or inst.components.health:IsDead() then return end
                    local clone = deps.SpawnPrefab(inst.prefab)
                    if clone then
                        local pt = inst:GetPosition()
                        local offset = deps.FindWalkableOffset(pt, math.random() * 2 * deps.PI, 2, 6, true, false)
                        clone.Transform:SetPosition((offset and (pt + offset) or pt):Get())
                        if clone.components.health then
                            local base_hp = inst.components.health.maxhealth or 100
                            clone.components.health:SetMaxHealth(base_hp * 0.5)
                            clone.components.health:SetPercent(1)
                        end
                        if clone.components.combat then
                            local base_dmg = inst.components.combat.defaultdamage or 20
                            clone.components.combat:SetDefaultDamage(base_dmg * 0.5)
                            local players = deps.CollectAlivePlayers()
                            if #players > 0 then
                                clone.components.combat:SetTarget(deps.PickRandom(players))
                            end
                        end
                        local fx = deps.SpawnPrefab("shadow_puff")
                        if fx then
        ...
        elseif chosen.id == "boss_drain_field" then
            -- 汲取领域：攻击时吸取目标生命并降低其治疗
            if not ent.rogue_boss_drain_hooked then
                ent.rogue_boss_drain_hooked = true
                ent:ListenForEvent("onhitother", function(inst, data)
                    local target = data and data.target
                    if target and target:IsValid() and target.components.health and inst.components.health then
                        local heal = (data.damage or 10) * 0.2
                        inst.components.health:DoDelta(heal, nil, "rogue_boss_drain")
                        if target.components.inventory and target.components.inventory.externalhealmodifiers then
                            target.components.inventory.externalhealmodifiers:SetModifier(target, 0.5, "rogue_boss_drain")
                            target:DoTaskInTime(5, function(t)
                                if t and t:IsValid() and t.components.inventory and t.components.inventory.externalhealmodifiers then
                                    t.components.inventory.externalhealmodifiers:RemoveModifier(t, "rogue_boss_drain")
                                end
                            end)
                        end
                    end
        ...
        elseif chosen.id == "boss_armor" then
            -- 不灭战甲：受到伤害-25%（通过 damage overrides 近似实现）
            if not ent.rogue_boss_armor_hooked then
                ent.rogue_boss_armor_hooked = true
                if ent.components.combat and ent.components.combat.externaldamagetakenmultipliers then
                    ent.components.combat.externaldamagetakenmultipliers:SetModifier(ent, 0.75, "rogue_boss_armor")
                end
            end
        elseif chosen.id == "boss_phase_3" then
            -- 三重暴走：血量低于30%时召唤精英护卫并狂暴
            if not ent.rogue_boss_phase3_hooked then
                ent.rogue_boss_phase3_hooked = true
                ent:ListenForEvent("healthdelta", function(inst)
                    if not inst:IsValid() or not inst.components.health or inst.components.health:IsDead() then return end
                    if inst.components.health:GetPercent() <= 0.3 and not inst.rogue_boss_phase3_triggered then
                        inst.rogue_boss_phase3_triggered = true
                        SpawnBossMinions(inst, 2 + math.min(2, math.floor((day - 1) / 35)))
                        if inst.components.combat and inst.components.combat.externaldamagemultipliers then
                            inst.components.combat.externaldamagemultipliers:SetModifier(inst, 1.3, "rogue_boss_phase3")
                        end
                        deps.Announce(deps.GetEntityName(inst) .. " 进入三重暴走！护卫被召唤，伤害提升！")
                    end
                end)
            end
        end

        ent:AddTag("rogue_boss_affix")
        ent.rogue_boss_affix_name = chosen.name
        if deps.Config.AFFIX_ANNOUNCE_MODE >= 1 then
            deps.Announce(deps.GetEntityName(ent) .. " 获得Boss词缀：【" .. chosen.name .. "】" .. chosen.desc)
        end
        return true
    end
    ...
    local function SetupBossPhases(ent, day)
        ...
        -- [V3 扩展] 尝试附加 Boss 专属词缀
        if day >= 20 then
            ApplyBossAffix(ent, day)
        end
    ...
    S.ApplyBossAffix = ApplyBossAffix
    return S
end
