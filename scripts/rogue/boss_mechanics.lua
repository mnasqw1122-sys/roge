...
function M.Create(deps)
    ...
    -- 函数说明：获取指定 Boss 变异为 V2 深度机制的概率
    -- v3.0 起从 defs/boss_balance_defs 读取：prefab 可单独覆盖，全局兜底 0.25
    local function GetVariantChance(prefab)
        local bal = deps.BossBalance or deps.Config and deps.Config.BOSS_BALANCE or {}
        local prefab_chance = bal.prefab_variant_chance and bal.prefab_variant_chance[prefab]
        return prefab_chance or bal.boss_variant_chance or 0.25
    end
    ...
    S.AttachBossTemplate = function(inst, day)
        ...
        if deps and deps.RogueBossMechanicsV2 then
            -- 如果是熊獾，有 GetVariantChance 概率变异为 V2 领域工匠机制
            if inst.prefab == "bearger" and math.random() < GetVariantChance("bearger") then
            ...
            -- 如果是树精守卫，有 GetVariantChance 概率变异为 V2 统御军势机制
            if (inst.prefab == "leif" or inst.prefab == "treeguard") and math.random() < GetVariantChance(inst.prefab) then
            ...
            -- 如果是鹿角鹅，有 GetVariantChance 概率变异为 V2 雷暴猎场机制
            if inst.prefab == "moose" and math.random() < GetVariantChance("moose") then
            ...
            -- 如果是蜘蛛女王，有 GetVariantChance 概率变异为 V2 双相裂变机制
            if inst.prefab == "spiderqueen" and math.random() < GetVariantChance("spiderqueen") then
            ...
            -- 如果是远古犀牛，有 GetVariantChance 概率变异为 V2 压迫领域机制
            if inst.prefab == "minotaur" and math.random() < GetVariantChance("minotaur") then
