...
-- [V2.3] 连击技能定义（与服务端 combo_system.lua 保持一致，供客户端展示与释放）
-- 释放按键 F5-F8，冷却由客户端本地模拟，服务端 RPC 二次校验
local COMBO_SKILLS = {
    { id = "shockwave", name = "冲击波", desc = "8格范围50伤害", combo_required = 30, cooldown = 60, key = "F5" },
    { id = "time_dilation", name = "时间膨胀", desc = "周围敌人减速50%5秒", combo_required = 50, cooldown = 90, key = "F6" },
    { id = "blood_fury", name = "嗜血狂怒", desc = "10秒伤害+25%吸血+10%", combo_required = 70, cooldown = 120, key = "F7" },
    { id = "thunder_strike", name = "雷霆一击", desc = "200%伤害闪电打击", combo_required = 100, cooldown = 150, key = "F8" },
}
...
local RogueStatusDisplay = Class(Widget, function(self, owner)
    ...
    self.key_handlers = {
        ...
        TheInput:AddKeyUpHandler(KEY_F5, function() self:TryReleaseComboSkill(1) end),
        TheInput:AddKeyUpHandler(KEY_F6, function() self:TryReleaseComboSkill(2) end),
        TheInput:AddKeyUpHandler(KEY_F7, function() self:TryReleaseComboSkill(3) end),
        TheInput:AddKeyUpHandler(KEY_F8, function() self:TryReleaseComboSkill(4) end),
    }
...
-- [V2.3] 释放连击技能（F5-F8）：客户端本地模拟冷却展示，服务端 RPC 校验
function RogueStatusDisplay:TryReleaseComboSkill(slot)
    if not self.owner or not self.owner:IsValid() then return false end
    local skill = COMBO_SKILLS[slot]
    if not skill then return false end
    local now = GetTime()
    local last = self.combo_skill_last_use and self.combo_skill_last_use[skill.id] or 0
    if now - last < skill.cooldown then
        ShowLocalHint(skill.name .. "冷却中（剩余" .. math.ceil(skill.cooldown - (now - last)) .. "秒）")
        return false
    end
    if MOD_RPC and MOD_RPC["rogue_mode"] and MOD_RPC["rogue_mode"]["combo_skill"] then
        self.combo_skill_last_use = self.combo_skill_last_use or {}
        self.combo_skill_last_use[skill.id] = now
        SendModRPCToServer(MOD_RPC["rogue_mode"]["combo_skill"], skill.id)
        return true
    end
    return false
end
...
function RogueStatusDisplay:RefreshFromNetvars()
    ...
    -- [V2.3] 连击技能栏：显示需求连击/冷却状态（F5-F8释放）
    local now_skill = GetTime()
    local skill_lines = {}
    for _, skill in ipairs(COMBO_SKILLS) do
        local last_use = self.combo_skill_last_use and self.combo_skill_last_use[skill.id] or 0
        local cd_left = math.ceil(skill.cooldown - (now_skill - last_use))
        local state_txt
        if cd_left > 0 then
            state_txt = "冷却" .. cd_left .. "s"
        elseif combo >= skill.combo_required then
            state_txt = "就绪"
        else
            state_txt = "需" .. skill.combo_required .. "连击"
        end
        table.insert(skill_lines, string.format("%s[%s]:%s", skill.key, skill.name, state_txt))
    end
    str = str .. "\n═══连击技能(F5-F8)═══\n" .. table.concat(skill_lines, " ")
