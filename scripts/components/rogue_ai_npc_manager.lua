...
local function PickPersonalityFromMemory(tactical_memory, level)
    ...
    if tactical_memory then
        ...
        elseif tactical_memory.last_fight_result == "timeout" then
            -- 上次因玩家久未应战而消散，下次倾向更激进以主动寻战
...
-- 函数说明：NPC长时间未被玩家攻击时调用（60秒未受伤害则主动离去）
-- 与 OnRestDayEnd/OnNPCKilled 的区别：NPC 既未死亡也未逃跑，因玩家久未应战而消散。
-- 清空 active_npc 以便下个生成日刷新新的挑战者；保留 saved_npc_data 让同一挑战者下天再临。
function RogueAINPCManager:OnNPCTimeout(npc)
    if self.active_npc == npc then
        self.active_npc = nil
    end
    ...
    if self.tactical_memory then
        self.tactical_memory.last_fight_result = "timeout"
    end

    if GLOBAL.TheNet then
        GLOBAL.TheNet:Announce("神秘的挑战者见无人应战，化作光影消散了...下个休息天它会再度现身。")
    end
end

-- 函数说明：休息天结束时调用（兜底清理：若 NPC 仍存活则消散离去，不再"逃跑"）
-- 注：NPC 现已不会因血量低逃跑，唯一离开方式为被击杀或 60 秒未受伤害消散；
-- 此处仅作为休息天切换的兜底清理，避免 NPC 跨天残留。
function RogueAINPCManager:OnRestDayEnd()
    if self.active_npc and self.active_npc:IsValid() then
        local pt = self.active_npc.Transform:GetWorldPosition()
        local fx = GLOBAL.SpawnPrefab("statue_transition_2")
        if fx and pt then
            fx.Transform:SetPosition(pt.x, pt.y, pt.z)
        end
        ...
        if GLOBAL.TheNet then
            GLOBAL.TheNet:Announce("休息天结束，神秘的挑战者化作光影消散了...下个休息天它会带着新的经验再度现身。")
        end
    end
end
